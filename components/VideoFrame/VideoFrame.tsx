...
const VideoFrame = ({ videoPath, totalFrames = 30, framesPath = null }: VideoFrameProps) => {
  ...
  useEffect(() => {
    ...
    pictureRefs.current = Array(imagesList.length)
      .fill(null)
      .map(() => createRef<HTMLPictureElement>());
        ...
        <div className="hidden-pictures">
          {images.map((image, index) => (
            <picture key={image.id} ref={pictureRefs.current[index]}>
              <img src={image.jpg} alt={`Frame ${image.id}`} className="preload-image" />
            </picture>
          ))}
        </div>
        ...
        {isLoading && (
          <div className="loading-overlay">
            <div className="loading-text">
              Loading frames: {imagesLoaded} / {totalFrames}
            </div>
