...
export default function TypeMeteorites({
  ...
  const itemTitleRef = useRef<(HTMLHeadingElement | null)[]>([]);
  const itemsTextRef = useRef<(HTMLDivElement | null)[]>([]);
  ...
  useEffect(() => {
    ...
    const update = () => {
      setR((containerRef.current?.offsetWidth ?? 0) / 2);
    };
    ...
    const ctx = gsap.context(() => {
      ...
      itemsRef.current.forEach((el, i) => {
        ...
        for (let step = 0; step <= 20; step++) {
          ...
          const angle =
            progress * rotations * Math.PI * 2 +
            (i * (Math.PI * 2)) / itemsRef.current.length;
        ...
        spiralTimeline.add(
          spiralTween,
          i === 0 ? 0 : "-=" + 0.95 * (staggerDelay * 5 + spiralTween.duration())
        );
      });
