...
export default function Intro({
  ...
  const meteor = useRef<HTMLDivElement>(null);
  const introBg = useRef<HTMLDivElement>(null);
  const introBgVideo = useRef<HTMLVideoElement>(null);
  ...
  const intro4Ref = useRef<HTMLDivElement>(null);
  const introText2Ref = useRef<HTMLDivElement>(null);
  const introText3Ref = useRef<HTMLDivElement>(null);
  const introText5Ref = useRef<HTMLDivElement>(null);
  const introText6Ref = useRef<HTMLDivElement>(null);
      ...
      mm.add(
        ...
        (context) => {
          ...
          tl.addLabel("intro-1")
            ...
            .set(intro2Ref.current, { opacity: 1 }, "-=.1");

          const decodeDuration2 =
            (introText2Ref.current!.textContent!.length - 1) * options.stagger +
            options.iterations * options.speed;
          const startDecode2 = tl.duration() - 0.1;
          tl.to(
            introBgVideo.current,
            {
              scale: 1.2,
          ...
          tl.addLabel("intro-2")
            ...
            .set(intro3Ref.current, { opacity: 1 });

          const decodeDuration3 =
            (introText3Ref.current!.textContent!.length - 1) * options.stagger +
            options.iterations * options.speed;
  ...
  return (
    ...
    >
      ...
      <div className="ia-container">
        <div
          className="intro-counter lg text--info text-light text-title"
          aria-live="polite"
          aria-atomic="true"
        >
          {progress}%
