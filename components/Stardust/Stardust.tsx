...
const Stardust = ({
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      const isOverSpline = (e.target as HTMLElement)?.closest(".stardust-page");
  ...
  return (
    <div id="Stardust">
      ...
      <div
        ref={sturdust}
        id="stardust-page"
        className={`stardust-page ${move ? "active scrolled" : ""} ${popupActive ? "scrolled" : ""}`}
      >
        <AnimatePresence>
          {popupActive && (
            <motion.div
            ...
            >
              <button
                className="stardust-page__popup-close"
                onClick={() => {
                  setMove(true);
                  setPopupActive(false);
                  gsap.to(window, {
                    duration: 0.5,
                    scrollTo: { y: sturdust.current ?? 0 },
                    ease: "power2.inOut",
                  });
                }}
                aria-label="close"
